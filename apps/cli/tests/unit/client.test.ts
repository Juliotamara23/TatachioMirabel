...
describe("api client — apiFetch", () => {
  ...
  it("prefixes base URL", async () => {
    const result = await apiFetch("/api/miembros", { baseUrl: BASE_URL, token: "valid-jwt" });
  ...
  it("throws ApiError on 401 with status=1", async () => {
    await expect(
      apiFetch("/api/miembros", { baseUrl: BASE_URL, token: "bad-token" }),
    ).rejects.toMatchObject({ status: 401 });
  });

  it("throws ApiError on 5xx with status=500", async () => {
    await expect(
      apiFetch("/api/crash", { baseUrl: BASE_URL, token: "valid-jwt" }),
  ...
  it("adds Content-Type: application/json by default for POST", async () => {
    ...
    await apiFetch("/api/echo", { method: "POST", baseUrl: BASE_URL, token: "valid-jwt", body: { x: 1 } });
